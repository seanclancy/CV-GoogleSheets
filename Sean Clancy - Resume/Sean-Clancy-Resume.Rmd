...
```{r results='asis'}
rbind(
    gsheet2tbl("https://docs.google.com/spreadsheets/d/1iwBWh1LUKUks0MBuS7vU8L0m35Qv70sb3Aa-84ybYBo/edit#gid=0") %>%
  filter(category == "peer-reviewed") %>%
    select(year, markdown),
  gsheet2tbl("https://docs.google.com/spreadsheets/d/1iwBWh1LUKUks0MBuS7vU8L0m35Qv70sb3Aa-84ybYBo/edit#gid=318960497") %>%
    slice(1:3) %>%
    select(year, markdown)
) %>%
  arrange(-year) %>%
  select(markdown) %>%
  mutate(markdown = str_replace_all(markdown, "Clancy, S.", "**Clancy, S.**")) %>%
  slice(1,2,4,6) %>%
  pull(markdown) -> intl
  cat(paste(intl), sep = '\n\n')
